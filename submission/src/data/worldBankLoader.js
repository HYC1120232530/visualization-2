...
export function buildWorldBankDataset({ workspaceRoot }) {
  // 提交包内 worldbank-csv/ 自包含；开发时仍可使用上级目录的作业原始 CSV
  const embeddedRoot = path.resolve(workspaceRoot, 'worldbank-csv')
  const parentRoot = path.resolve(workspaceRoot, '..')
  const firstMetricPath = path.join(embeddedRoot, METRIC_SOURCES[0].folderName)
  const assignmentRoot = fs.existsSync(firstMetricPath) ? embeddedRoot : parentRoot
